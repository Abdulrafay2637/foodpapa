  ...
  querySnapshot.forEach((doc) => {
    ...
    card.innerHTML = `
      <div class="d-flex justify-content-between align-items-center mb-2">
        <h6 class="mb-0">Item Total: Rs. <span class="item-total">0</span></h6>
        <button class="btn-close"></button>
      </div>
      <img src="${data.url}" class="card-img-top mb-2" alt="${data.name}">
      <div class="card-body p-0">
        <h5 class="card-title">${data.name}</h5>
        <p class="card-text">${data.description || ""}</p>
        <p class="mb-2">Price per item: Rs. ${price}</p>
      </div>
    ...
    const itemTotalSpan = card.querySelector(".item-total");
    const closeBtn = card.querySelector(".btn-close");

    plusBtn.addEventListener("click", () => {
      itemTotal += price;
      totalPrice += price;
      itemTotalSpan.innerText = itemTotal;
    ...
    closeBtn.addEventListener("click", () => {
      totalPrice -= itemTotal; // Remove this card's total from overall total
      totalPriceSpan.innerText = totalPrice;
      card.remove();
    });
